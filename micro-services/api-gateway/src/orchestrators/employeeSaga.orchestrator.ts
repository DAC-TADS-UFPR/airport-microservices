...
export class EmployeeSagaOrchestatorator {    
    ...
    public async createEmployee(employeeData: IEmployee): Promise<any> {
        try {
            const employeeResponse = await axios.post(`${SERVICE_CONFIG.EMPLOYEE.url}/funcionarios`, employeeData);
            const employeeId = employeeResponse.data?.id;
            if(employeeResponse.status !== 201) {
                return employeeResponse;
            }
            const authRequest:IAuth = new IAuth( employeeData.email, '1234', employeeData.name ,employeeId, UserType.EMPLOYEE);
